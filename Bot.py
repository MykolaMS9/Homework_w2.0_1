...
import sys
from contacts_core import AddressBook, Record, ContactExist, \
    ContactNotExist, UncorrectedPhoneNumber, TypeValue, UncorrectedBirthdayType, UnknownCommand
...
class AddPhone(NewCommand):
    ...
    @input_error
    def __call__(self, contact_name, phone_number,  *args, data: AddressBook, **kwargs):
...
class AddBirthday(NewCommand):
    ...
    @input_error
    def __call__(self, contact_name, birthday_date,  *args, data: AddressBook, **kwargs):
...
class Change(NewCommand):
    ...
    @input_error
    def __call__(self, contact_name, exist_phone, phone_number,  *args, data: AddressBook, **kwargs):
...
class Phone(NewCommand):
    ...
    @input_error
    def __call__(self, contact_name, *args, data: AddressBook, **kwargs):
        if not contact_name:
            raise TypeValue
        if contact_name in data.data:
...
class ShowAll(NewCommand):
    ...
    @input_error
    def __call__(self, n, *args, data: AddressBook, **kwargs):
        if not n:
            n = 1
...
class Search(NewCommand):
    ...
    @input_error
    def __call__(self, need_find: str, *args, data: AddressBook, **kwargs):
        flag = False
...
class Help(NewCommand):
    ...
    @input_error
    def __call__(self, *args, data, commands_dict, **kwargs):
...
class Handler:
    ...
    def __init__(self):
        commands_list = [Close(), Hello(), AddPhone(), AddBirthday(), Change(), Phone(), ShowAll(), Search(), Delete(),
                         Help()]
        for command_obj in commands_list:
            for command in command_obj.command:
                self.dict_[command] = command_obj

    def __find_command(self, string) -> tuple:
    ...
    @input_error
    def __call__(self, input_string: str, data: object):
        (command_name, arguments) = self.__find_command(input_string)
        if not command_name in self.dict_:
            raise_error()
        command = self.dict_.get(command_name)
        values = 3*[None]
        for val, arg in zip(values, arguments):
            values[values.index(val)] = arg
        return command(values[0], values[1], values[2], data=data, commands_dict=self.dict_)
